(function () {
  ...
  function htmlToMarkdown(html) {
    ...
    cleanedHtml = cleanedHtml.replace(
      /<ul[^>]*>([\s\S]*?)<\/ul>/gi,
      function (match, ulContent) {
        return ulContent.replace(
          /<li[^>]*>([\s\S]*?)<\/li>/gi,
          (liMatch, liContent) => `- ${liContent.trim()}\n`
        );
      }
    );
    cleanedHtml = cleanedHtml.replace(
      /<ol[^>]*>([\s\S]*?)<\/ol>/gi,
      function (match, olContent) {
        let counter = 1;
        return olContent.replace(
          /<li[^>]*>([\s\S]*?)<\/li>/gi,
          (liMatch, liContent) => `${counter++}. ${liContent.trim()}\n`
        );
      }
    );
    cleanedHtml = cleanedHtml.replace(
      /<div[^>]*class="[^"]*r-adyw6z[^"]*"[^>]*>([\s\S]*?)<\/div>/gi,
      "\n## $1\n"
    );
    cleanedHtml = cleanedHtml.replace(
      /<div[^>]*class="[^"]*r-b88u0q[^"]*"[^>]*>([\s\S]*?)<\/div>/gi,
      "**$1**\n"
    );
    cleanedHtml = cleanedHtml.replace(/<p[^>]*>([\s\S]*?)<\/p>/gi, "$1\n\n");
    cleanedHtml = cleanedHtml.replace(
      /<div[^>]*class="[^"]*r-1qd0xha[^"]*"[^>]*>([\s\S]*?)<\/div>/gi,
      "$1\n\n"
    );
  ...
  function copyAsMarkdown(event) {
    ...
    console.log(
      "LOG: Copy button clicked. Button HTML:",
      copyButton.outerHTML.substring(0, 200) + "..."
    );

    const actionBar = copyButton.closest(
      'div[class*="r-18u37iz"][class*="r-1wtj0ep"]'
    );
    if (!actionBar) {
    ...
    console.log(
      "LOG: ActionBar found. HTML:",
      actionBar.outerHTML.substring(0, 200) + "..."
    );
    ...
    while (
      currentElementForMessageBlockSearch &&
      currentElementForMessageBlockSearch.parentElement &&
      iterationCount < MAX_ITERATIONS
    ) {
      let parent = currentElementForMessageBlockSearch.parentElement;
      console.log(
        `LOG: Checking parent [${iterationCount}] for messageBlock:`,
        parent.className,
        "HTML:",
        parent.outerHTML.substring(0, 200) + "..."
      );
      ...
      if (
        contentCandidate &&
        actionBarWrapperCandidate &&
        contentCandidate !== actionBarWrapperCandidate
      ) {
        messageBlock = parent;
        console.log(
          "LOG: Found messageBlock:",
          messageBlock.className,
          "Contains distinct content and action bar wrappers."
        );
        console.log(
          "LOG:   Content Candidate in messageBlock:",
          contentCandidate.className
        );
        console.log(
          "LOG:   ActionBar Wrapper in messageBlock:",
          actionBarWrapperCandidate.className
        );
        break;
      }
    ...
    if (!messageBlock) {
      console.log(
        "LOG: Could not find a suitable messageBlock after traversing up."
      );
      return;
    }
    console.log(
      "LOG: Final messageBlock determined:",
      messageBlock.className,
      "HTML:",
      messageBlock.outerHTML.substring(0, 300) + "..."
    );
    ...
    console.log(
      "LOG: Searching for contentWrapper within messageBlock. Number of children:",
      messageBlock.children.length
    );
    for (const child of messageBlock.children) {
      console.log(
        "LOG: Checking messageBlock child: className=`" +
          child.className +
          "`, tagName=`" +
          child.tagName +
          "`. Is it or does it contain actionBar?",
        child === actionBar || child.contains(actionBar)
      );
      if (
        child.tagName === "DIV" &&
        !(child === actionBar || child.contains(actionBar))
        ...
        if (
          ...
          console.log(
            "LOG: contentWrapper candidate found by specific class (r-1wbh5a2) and content:",
        ...
        if (
          ...
          contentWrapper = child;
          console.log(
            "LOG: contentWrapper candidate (general content markers):",
            contentWrapper.className
          );
        }
        if (!contentWrapper) {
          contentWrapper = child;
          console.log(
            "LOG: contentWrapper candidate (generic fallback):",
            contentWrapper.className
          );
        }
      }
    ...
    if (contentWrapper) {
      console.log(
        "LOG: contentWrapper found:",
        contentWrapper.className,
        "HTML:",
        contentWrapper.outerHTML.substring(0, 300) + "..."
      );
      ...
      if (intermediateContainer && intermediateContainer.tagName === "DIV") {
        console.log(
          "LOG: intermediateContainer found:",
          intermediateContainer.className,
          "HTML:",
          intermediateContainer.outerHTML.substring(0, 200) + "..."
        );
        const potentialAnswerContainer =
          intermediateContainer.firstElementChild;
        if (
          potentialAnswerContainer &&
          potentialAnswerContainer.tagName === "DIV" &&
          potentialAnswerContainer.classList.contains("css-175oi2r") &&
          potentialAnswerContainer.style.display === "block" && // Crucial style check
          potentialAnswerContainer.querySelector(
            ':scope > div[dir="ltr"], :scope > ul, :scope > ol, :scope > hr, :scope > pre, :scope > p'
          )
        ) {
          answerContainer = potentialAnswerContainer;
          console.log(
            "LOG: answerContainer found by specific path:",
          ...
          console.log(
            "LOG: Specific path for answerContainer failed. Potential AC:",
            potentialAnswerContainer
              ? potentialAnswerContainer.className
              : "null",
            "Style:",
            potentialAnswerContainer
              ? potentialAnswerContainer.style.cssText
              : "null",
            "Content markers?:",
            potentialAnswerContainer
              ? !!potentialAnswerContainer.querySelector(
        ...
        console.log(
          "LOG: intermediateContainer (contentWrapper.firstElementChild) not found or not a DIV. Found:",
      ...
      if (!answerContainer) {
        console.log(
          "LOG: Attempting general search for answerContainer within contentWrapper..."
        );
        const candidates = contentWrapper.querySelectorAll(
          'div.css-175oi2r[style*="display: block;"]'
        ); // More specific class
        ...
        for (const cand of candidates) {
          if (
            ...
            console.log(
              "LOG: answerContainer found by general search:",
              answerContainer.className,
              "Style:",
              answerContainer.style.cssText
            );
    ...
    if (!answerContainer) {
      ...
      if (contentWrapper)
        console.log(
          "LOG: contentWrapper existed. HTML(first 500):",
          contentWrapper.innerHTML.substring(0, 500)
        );
      else if (messageBlock)
        console.log(
          "LOG: messageBlock existed. HTML(first 500):",
          messageBlock.innerHTML.substring(0, 500)
        );
      return;
    }
    console.log(
      "LOG: answerContainer successfully located:",
      answerContainer.className,
      "HTML:",
      answerContainer.outerHTML.substring(0, 300) + "..."
    );
    ...
    if (answerContainer.children.length > 0) {
      ...
      console.log(
        "LOG: Extracted HTML from answerContainer children. Count:",
        answerContainer.children.length
      );
    } else if (
      answerContainer.textContent &&
      answerContainer.textContent.trim() !== ""
    ) {
    ...
    if (!answerHtml.trim()) {
      console.log(
        "LOG: Extracted answerHtml is empty. answerContainer HTML preview:",
        answerContainer.innerHTML.substring(0, 500)
      );

      const rawText =
        answerContainer.innerText || answerContainer.textContent || "";
      if (rawText.trim()) {
        navigator.clipboard
          .writeText(rawText.trim())
          .then(() => {
            console.log(
              "LOG: Copied raw text because extracted HTML for markdown was empty."
            );
          })
          .catch((err) => {
    ...
    if (!markdownContent.trim()) {
      console.warn(
        "LOG: Markdown conversion resulted in empty string. Original HTML preview:",
        answerHtml.substring(0, 500) + "..."
      );
      const rawText =
        answerContainer.innerText || answerContainer.textContent || "";
      navigator.clipboard
    ...
    navigator.clipboard
      ...
      .then(() => {
        console.log(
          "LOG: Successfully copied Markdown to clipboard. Preview:",
